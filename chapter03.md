...
**填表举例**：5 个矩阵，维度向量 $p = [5, 10, 3, 12, 5, 50]$，即 $A_1: 5 \times 10$，$A_2: 10 \times 3$，$A_3: 3 \times 12$，$A_4: 12 \times 5$，$A_5: 5 \times 50$。

| $C[i,j]$ | 1 | 2 | 3 | 4 | 5 |
|-----------|:-:|:-:|:-:|:-:|:-:|
| **1** | 0 | 150 | 330 | 580 | **1830** |
| **2** | — | 0 | 360 | 330 | 2430 |
| **3** | — | — | 0 | 180 | 930 |
| **4** | — | — | — | 0 | 3000 |
| **5** | — | — | — | — | 0 |
...
- $L = 1$（主对角线）：$C[i,i] = 0$
- $L = 2$（相邻两个矩阵直接相乘）：
  - $C[1,2] = 5 \times 10 \times 3 = 150$
  - $C[2,3] = 10 \times 3 \times 12 = 360$
  - $C[3,4] = 3 \times 12 \times 5 = 180$
  - $C[4,5] = 12 \times 5 \times 50 = 3000$
- $L = 3$（开始体现 DP 的优势——复用子问题）：
  - $C[1,3]$：$k=1$: $0 + 360 + 5 \times 10 \times 12 = 960$；$k=2$: $150 + 0 + 5 \times 3 \times 12 = 330$ ✓
  - $C[2,4]$：$k=2$: $0 + 180 + 10 \times 3 \times 5 = 330$ ✓；$k=3$: $360 + 0 + 10 \times 12 \times 5 = 960$
  - $C[3,5]$：$k=3$: $0 + 3000 + 3 \times 12 \times 50 = 4800$；$k=4$: $180 + 0 + 3 \times 5 \times 50 = 930$ ✓
- $L = 4$：
  - $C[1,4]$：$k=1$: $0 + 330 + 5 \times 10 \times 5 = 580$ ✓；$k=2$: $150 + 930 + 75 = 1155$；$k=3$: $330 + 0 + 300 = 630$
  - $C[2,5]$：$k=2$: $0 + 930 + 10 \times 3 \times 50 = 2430$ ✓；$k=3$: $360 + 3000 + 6000 = 9360$；$k=4$: $330 + 0 + 2500 = 2830$
- $L = 5$（最终答案）：
  - $C[1,5]$：$k=1$: $0 + 2430 + 2500 = 4930$；$k=2$: $150 + 930 + 750 = 1830$ ✓；$k=3$: $330 + 3000 + 3000 = 6330$；$k=4$: $580 + 0 + 1250 = 1830$ ✓

最优代价为 $C[1,5] = 1830$。$k=2$ 对应方案 $(A_1 A_2)(A_3 A_4 A_5)$，其中 $A_3 A_4 A_5$ 进一步按 $k=4$ 分割为 $(A_3 A_4) A_5$。
...
**问题定义**：Fab 数是 Fibonacci 数列在二维网格上的推广（来自 Recitation 04 习题）。在一个 $n \times n$ 的网格中，每个格子 $Fab(i, j)$ 的值由相邻格子决定，规则因位置而异：

**Base Cases**：$Fab(1, 1) = Fab(1, 2) = Fab(2, 1) = 1$。
...
- **第 1 行**（$i = 1$）：退化为一维 Fibonacci，每个格子等于左边两个格子之和
- **第 1 列**（$j = 1$）：类似一维递推，但依赖关系略有不同（$Fab(i-1, 1) + Fab(i-2, 1)$ 对 $i \ge 3$；$Fab(2, 1) = 1$ 由 Base Case 给出）
- **内部格子**（$i \ge 2, j \ge 2$）：等于**上方** + **左方**，与网格路径计数问题的递推完全一致

**填表举例**：$n = 4$ 时的 Fab 表格。

| $Fab(i,j)$ | $j=1$ | $j=2$ | $j=3$ | $j=4$ |
|-------------|:-----:|:-----:|:-----:|:-----:|
| $i=1$ | 1 | 1 | 2 | 3 |
| $i=2$ | 1 | 2 | 4 | 7 |
| $i=3$ | 2 | 4 | 8 | 15 |
...
- 第 1 行：$Fab(1,3) = 1 + 1 = 2$，$Fab(1,4) = 2 + 1 = 3$（一维 Fibonacci）
- 第 1 列：$Fab(3,1) = Fab(2,1) + Fab(1,1) = 1 + 1 = 2$，$Fab(4,1) = 2 + 1 = 3$
- 内部：$Fab(2,2) = Fab(1,2) + Fab(2,1) = 1 + 1 = 2$，$Fab(4,4) = Fab(3,4) + Fab(4,3) = 15 + 15 = 30$
